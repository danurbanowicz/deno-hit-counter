...
app.use(async (ctx, next) => {
  ...
  // Proceed to next middleware for background logging
  await next();
});

// Capture visit data and write to the DB (runs after pixel is served)
app.use(async (ctx) => {
  // Run visit logging asynchronously without blocking
  (async () => {
    ...
    console.log("Background visit logging error:", error);
  });
});
